...
demo_scene :: proc() -> geo_layers.Scene {
	...
	ops_sites: geo_layers.FeatureLayer
	ops_sites.base = {kind = .Feature, name = "Ops Sites", visible = true}
	append(&ops_sites.features, geo_layers.Feature{id = 1, name = "North Harbor Node", position = geo_core.LatLon{lat =  37.7749, lon = -122.4194}, elevation_m =  18.0, category = .Facility,    color = {0.95, 0.76, 0.35, 1}})
	append(&ops_sites.features, geo_layers.Feature{id = 3, name = "Coastal Ops Cell",  position = geo_core.LatLon{lat =  47.6062, lon = -122.3321}, elevation_m =  60.0, category = .City,        color = {0.58, 0.95, 0.46, 1}})
	append(&ops_sites.features, geo_layers.Feature{id = 4, name = "Forward Cache",     position = geo_core.LatLon{lat =  35.6895, lon =  139.6917}, elevation_m =  40.0, category = .Route_Point, color = {0.98, 0.42, 0.28, 1}})
	append(&ops_sites.features, geo_layers.Feature{id = 5, name = "Imagery Index",     position = geo_core.LatLon{lat =  51.5074, lon =   -0.1278}, elevation_m =  35.0, category = .Note,        color = {0.85, 0.78, 0.98, 1}})
	geo_layers.scene_add_feature_layer(&s, ops_sites)

	sensor_net: geo_layers.FeatureLayer
	sensor_net.base = {kind = .Sensor, name = "Sensor Net", visible = true}
	append(&sensor_net.features, geo_layers.Feature{id = 2, name = "Ridge Relay",     position = geo_core.LatLon{lat =  34.0522, lon = -118.2437}, elevation_m = 420.0, category = .Sensor, color = {0.33, 0.85, 0.92, 1}})
	append(&sensor_net.features, geo_layers.Feature{id = 6, name = "Southern Sensor", position = geo_core.LatLon{lat = -33.8688, lon =  151.2093}, elevation_m =  22.0, category = .Sensor, color = {0.72, 0.91, 1.00, 1}})
	geo_layers.scene_add_feature_layer(&s, sensor_net)
